...
export function CommanderPathBar({
  ...
  return (
    <div
      className={[
        'flex h-9 shrink-0 items-center gap-2 overflow-hidden border-b px-3 font-mono text-xs text-muted-foreground transition-colors',
    ...
    >
      ...
      {isPathEditing ? (
        <input
          className="h-7 min-w-0 flex-1 rounded border border-primary/60 bg-slate-950 px-2 text-xs text-foreground outline-none"
          autoFocus
          value={pathDraft}
          onBlur={() => {
            onPathDraftChange(displayPath);
        ...
        <div className="app-scrollbar flex min-w-0 items-center gap-1 overflow-x-auto overflow-y-hidden whitespace-nowrap">
          {variant === 'local' && firstPathSegment && localRoots.length > 0 && (
            <select
              className="max-w-28 shrink-0 rounded border border-transparent bg-slate-950/40 px-1.5 py-0.5 font-mono text-xs text-foreground outline-none hover:border-border/70 hover:bg-slate-900/80 focus:border-primary/60"
          ...
          {breadcrumbSegments.length > 0 ? breadcrumbSegments.map((segment, index) => {
              ...
              <span className="flex min-w-0 items-center gap-1" key={`${segment.path}-${index}`}>
                ...
                <button
                  className={[
                    ...
                    isCurrentPath ? 'cursor-default text-foreground' : 'text-muted-foreground',
                  ].join(' ')}
                  type="button"
                  title={segment.path}
            ...
            <button
              className="min-w-0 flex-1 truncate rounded px-1.5 py-0.5 text-left text-muted-foreground hover:bg-slate-900/70 hover:text-foreground"
      ...
      {onBrowseDirectory && (
        <button
          className="ml-1 grid size-7 shrink-0 place-items-center rounded text-slate-500 hover:bg-slate-900/70 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
          type="button"
