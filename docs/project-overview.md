...
이 문서는 새 Codex 스레드나 사용자가 ShellPilot 전체 구조를 빠르게 파악하기 위한 한국어 인덱스입니다. 기능별 세부 설계는 `rdp-handoff.md`, `sftp-handoff.md`, `ai-tool-layer-design.md`를 함께 봅니다.

## 현재 제품 방향

ShellPilot은 원격 작업용 통합 워크스페이스입니다.

- 왼쪽 Activity/Sidebar에서 세션, 열린 탭, SFTP 탐색기, AI, 로그 영역을 전환합니다.
- 중앙은 FlexLayout 기반 워크스페이스이며 SSH, SFTP, RDP, 로컬 터미널, AI, Settings 패널이 탭으로 열립니다.
...
src/main.tsx
  -> App.tsx
     ...
     -> Workspace
        -> createPanelFactory
           -> PanelBody
              -> SshTerminal | LocalPtyTerminal | SftpPanel | RdpPanel | AiPanel | SettingsPanel
```
...
자세한 내용은 `docs/rdp-handoff.md`. 더 긴 결정 로그는 `docs/rdp-design.md`에 남아 있습니다.
...
자세한 내용은 `docs/ai-tool-layer-design.md`.
...
- AI/SSH/SFTP/settings/workspace는 서로 얽힌 영역입니다. 기능별 작업 중 공통 통합 지점을 수정할 수는 있지만, 목적 없는 정리는 피합니다.
- RDP sidecar의 `main.rs`는 아직 큽니다. 동작이 안정화된 뒤 frame/display/input/clipboard/bootstrap 단위로만 의미 있게 분리합니다.
...
2. 관련 문서 확인
   - 전체 구조: 이 문서
   - 기능별: `rdp-handoff.md`, `sftp-handoff.md`, `ai-tool-layer-design.md`
3. 관련 파일만 `rg`로 찾기
4. 작은 동작 단위로 수정
5. 관련 check 실행
